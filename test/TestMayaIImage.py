...
_filename = "C:/Users/siras/Desktop/test_playblast/up.jpg"
...
class MayaImage :
	...
	def __init__(self,filename = None, w=0, h=0) :
		...
		if filename:
			...
			self.image.create(w, h, 4, OpenMaya.MImage.kByte )
	...
	def saveToFile(self, path, outputFormat = "jpg"):
		"""Save to file"""
		self.image.writeToFile( pathname = path, outputFormat = outputFormat )
		return path

if __name__ == '__main__':
	
	img=MayaImage(filename = _filename)
	print img.width()
	print img.height()
	xoffset = -img.width()/2
