...
def combineToCube( left, front, right, back, top, bottom, in_widht, in_height, output_path ):
	"""
	...
	o_img = MayaImage()
	o_img.create (w = out_widht, h = out_height)
	...
	for index, imageFile in enumerate([ left, front, right, back, top, bottom ]) :
		...
		for y in range (0,img.height()) :
			...
			for x in range(0,img.width()) :
				...
				o_img.setPixel( x = x + x_offset, y = y + y_offset, color = [r,g,b])
...
if __name__ == '__main__':
	...
	combineToCube( 	left 	= imageList[3],
					front 	= imageList[2], 
					right 	= imageList[4], 
					back 	= imageList[0], 
					top 	= imageList[5], 
